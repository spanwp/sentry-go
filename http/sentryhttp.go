...
type Handler struct {
	repanic              bool
	recoveredErrorLogger func(err any)
...
type Options struct {
	...
	Repanic bool
	// RecoveredErrorLogger is called when an error is recovered from a panic in the HTTP handler.
	// It allows you to log the error to your logging system.
	RecoveredErrorLogger func(err any)
...
func New(options Options) *Handler {
	...
	return &Handler{
		...
		recoveredErrorLogger: options.RecoveredErrorLogger,
...
func (h *Handler) recoverWithSentry(hub *sentry.Hub, r *http.Request) {
	if err := recover(); err != nil {
		...
		if h.recoveredErrorLogger != nil {
			h.recoveredErrorLogger(err)
		}
		if h.repanic {
			panic(err)
		}
